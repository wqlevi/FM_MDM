...
def main(rank, world_size, *args, **kwargs):
    ...
    model = UNetModel(dim=(3, IMG_SIZE, IMG_SIZE), num_channels=128, num_res_blocks=2,  **model_arch_dict).to(rank) 
    ...
    for it in range(ITERS):
        ...
        for _ in range(n_inner_loop):
            ...
            loss_v = torch.stack([torch.mean((vt - ut)**2) for vt, ut in zip(vt_ls, ut_ls)]).mean()
            loss_v.backward()
...
def evaluate(rank, dataloader, node, corrupt_method_all_res, res_group, it):
    ...
    def plot_result(x:List[Tensor], caption:str=None):
        titles = ['x0', 'x1', 'x0_ode'] 
        assert isinstance(x[0], Tensor), "Should been Tensor but got {}".format(type(x[0]))
        grids = [make_grid(xi[:100].clip(-1, 1),value_range=(-1,1), padding=0, nrow=10) for xi in x] if isinstance(x, list) else make_grid(x[:100].clip(-1,1), value_range=(-1,1), padding=0, nrow=10)

...
def main(opt):
    ...
    run.train(opt, train_dataset, val_dataset, corrupt_method) #[FIXME] l153 in runner.py
    log.info("Finish!")
...
if __name__ == '__main__':
    ...
    if opt.distributed:
        ...
        for rank in range(size):
            ...
            print('\033[93m (hard-coded)\033[0m Node rank %d, local proc %d, global proc %d, global_size %d' % (opt.node_rank, rank, global_rank, global_size))
            p = Process(target=init_processes, args=(global_rank, global_size, main, opt)) #[FIXME] CUDA init error for line:142 
            p.start()
            processes.append(p)

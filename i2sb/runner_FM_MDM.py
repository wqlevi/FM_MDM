...
class Runner(object):
    def __init__(self, opt, log, save_opt=True):
        ...
        self.net = nestedUnet(opt.device, min_size=64) 
    ...
    def sample_batch(self, opt, loader, corrupt_method, resize:int):
    ...
    def train(self, opt, train_dataset, val_dataset, corrupt_method):
        ...
        for it in range(self.current_iter, opt.num_itr):
            ...
            for _ in range(n_inner_loop): # only cumulate gradients here:
                ...
                tmp_ls = [self.sample_batch(opt, train_loader, corrupt_method, res) for corrupt_method, res in zip(corrupt_method_all_res, res_group)]
                ...
                loss = loss_0 + loss_1 + loss_2 if not opt.use_weighting else loss_0 + 2*loss_1 + 4*loss_2
                loss.backward()
                loss_v0 += loss_0.item()
                loss_v1 += loss_1.item()
                loss_v2 += loss_2.item()
    ...
    def evaluation(self, opt, it, val_loader, corrupt_method_all_res, res_group):
        ...
        val_ls = [self.sample_batch(opt, val_loader, corrupt_method, res) for corrupt_method, res in zip(corrupt_method_all_res, res_group)]

...
class ProUNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd(dims, in_channels, self.ch, 3, padding=1))]
        )
    ...
    def forward(self, t, x, y=None, step=0):
        ...
        if step == 1:
            for module in self.output_step_layers:
    ...
    # TODO: define downsample, upsample, mid, skip_connection foward function
    
    def f_up(self, x:Tensor, t:Tensor, channel_mult:List[int], block_channels:List[Tensor])-> Tensor:
        """
        x: input tensor
        channel_mult: numbers of repetition of channel per layer
        block_channels: numbers of channel per layer
        """
        ich = block_channels.pop()
        mult = channel_mult.pop() # pop once each func call
        layers = [
            ResBlock(self.ch + ich,
                     self.time_embed_dim,
                     self.dropout,
        ...
        out_ch = self.ch
        layers.append(Upsample(self.ch, self.conv_resample, dims = self.dims, out_channels=out_ch))
        self.ds //=2
        layer_module = TimestepEmbedSequential(*layers) # TimestepEmbedSequential is a wrapper for t and x input
        return layer_module(x, t) 

    def forward_nested_UNet(self,x:List[Tensor], h:Tensor=None, o=[])-> List[Tensor]:
        """
        f_up, f_down, f_mid, f_skip can be nn.Module instance
        """
        z = x[-1] if h is None else x[-1]+h
        if len(x) > 1:
            z = self.f_up(self.forward_nested_UNet(x[:-1], self.f_down(z), o))
        else:
            z = self.f_up(self.f_mid(self.f_down(z)))
        o.append(z)
        return z
    ...
    def make_up_down_blocks(self, block_layers: nn.ModuleList,
        ...
        for level, mult in enumerate(channel_mult): 
            for i in range(self.num_res_blocks + res_shift):
                ...
                if not upsample: block_layers.append(TimestepEmbedSequential(*layers)) 

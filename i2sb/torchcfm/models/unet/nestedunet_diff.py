...
def print_shape_wrapper(layer_name:str=""):
    def wrapper_fn(fn):
        def inner_fn(self, *args):
            [print(f"\033[93mtensor input shape: {arg.shape}\033[0m") if isinstance(arg, torch.Tensor) else print(f"\033[91m---\n{layer_name} Layer shape: {arg}\n---\033[0m") for arg in args]
            out = fn(self, *args)
...
class ProUNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = self.input_blocks[::-2]
        #self.output_blocks = self.output_blocks[::2] # upsample per element
        self.output_blocks = self.output_blocks[::-2] # upsample per element
    ...
    def _inference_downsample(self, x:Tensor, min_size:int=8)->List[Tensor]:
        ...
        #return [l for sb in ls for l in sb] # FIXME DUMB design: Tensor is a List obj it self!
        return [*ls_down, x]
    ...
    def forward(self, t:Tensor, x:List[Tensor],*args, **kwargs ) -> List[Tensor]:
        ...
        # -[x]: print layers used at inference
        # FIXME: the integration only takes one resolution, which causes 16x16 to use 8x8 layers; change it to use a list of downsampling of the largest resolution
        # TODO: generate list of downsampling images here at inference time
    ...
    def hook_fn(self, m:nn.Module, i:Tuple[Tensor, Tensor] ,o:Tensor):
        ...
        wandb.log({block_res+'input mean': i[0].mean(),
                   block_res+'output mean': o.mean()})
        _get_weight_mean(m)
        #print(f"Mean of output tensors: {o.mean():.3f}")
